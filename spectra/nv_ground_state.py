...
if __name__ == "__main__":
    ...
    static_bvec = bo * np.array([0, 1., 1.])
    transition_bvec = np.array([0., 1.E-6, 0])
    initial_populations = np.array([1., 0, 0, 0, 0, 0, 0, 0, 0])
    plot_nv_ground_magnetic_transition_amplitudes(transition_bvec, static_bvec, initial_populations=initial_populations)
    frequencies = np.linspace(2.8E9, 3.E9, num=2**16)
    spectrum = get_power_broadened_spectrum(frequencies, transition_bvec, static_bvec,
                                            initial_populations=initial_populations)
    fig, ax = plt.subplots(1, 1)
    ax.grid(True)
    ax.set_xlabel('frequency (MHz)')
    ax.set_ylabel('Intensity (normalized)')
    ax.plot(frequencies * 1.E-6, spectrum / np.max(spectrum))
    plt.show()
